...
export default function UploadPage() {
  ...
  const [preview, setPreview] = useState(""); // full data URL including prefix
  ...
  const analyzeImage = async () => {
    ...
    try {
      // 1️⃣ Call your HuggingFace model
      const modelResult = await callHuggingFaceModel(preview);
      setProgress(65);

      // 2️⃣ Save everything to Firestore "cases" collection
      setStatus("saving");
      ...
      const docRef = await addDoc(collection(db, "cases"), {
        ...
        originalImageBase64: preview,                      // user's upload
        annotatedImageBase64: modelResult.annotatedImageBase64, // YOLO result
        gradCamBase64: modelResult.gradCamBase64,          // heatmap
      ...
      // 3️⃣ Redirect to /results (fetches latest case for this user)
      router.push("/results");
      ...
      <div className="w-full max-w-xl bg-white rounded-[28px] shadow-[var(--shadow-card)] p-6 sm:p-10 flex flex-col gap-6">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-soft)]">
            Upload a pediatric wrist X-ray. The AI model will detect fractures and
        ...
        {preview && !isLoading && (
          <button
            onClick={() => { setFile(null); setPreview(""); setError(""); setStatus("idle"); }}
        ...
        <div className="flex justify-between text-sm text-[var(--text-soft)]">
          <button onClick={() => router.push("/dashboard")} className="hover:text-[var(--primary)]">
            ← Dashboard
          </button>
          <button onClick={() => router.push("/results")} className="hover:text-[var(--primary)]">
            View Last Result →
          </button>
        </div>
      </div>
